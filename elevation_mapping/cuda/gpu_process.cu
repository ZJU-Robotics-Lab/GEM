...
{    
	...
	if (i < Length * shift) {
        if(row_flag){
            ...
            map_intensity[start * Length + i] = -10;
            ...
            map_intensity[i / shift * Length + i % shift + start] = -10;
...
__global__ void G_pointsprocess(int* map_index, float *point_x, float *point_y, float *point_z, float *result_var, float *point_x_ts, float *point_y_ts, float *point_z_ts, Eigen::Matrix4f transform, int point_num, Eigen::RowVector3f C_sensorJacobian, Eigen::Matrix3f C_rotationVariance, Eigen::Matrix3f C_C_SB_transpose, Eigen::RowVector3f C_P_mul_C_BM_transpose, Eigen::Matrix3f C_B_r_BS_skew)
{
	...
	if(i < point_num)
	{
		...
		if((point_x[i] > -0.5 && point_x[i] < 0.7 && point_y[i] > -0.5 && point_y[i] < 0.5))
		{
			flag = 1;
		}
...
__global__ void G_fuse(int *point_index, int *points_colorR, int *points_colorG, int *points_colorB, float *points_h, float *points_v, float *points_i, int point_num){
    ...
    if(map_index < Length * Length){
       ...
       for(int i = 0; i < point_num; i++)
       {
            ...
            if (map_elevation[map_index]== -10){
                ...
                map_intensity[map_index] = points_i[i];
                if(points_colorR[i] != 0 && points_colorG[i] != 0 && points_colorB[i] != 0)
                {
...
__global__ void G_Mapfeature(int *d_colorR, int *d_colorG, int *d_colorB, float *d_elevation, float *d_var, float *d_intensity, float *d_rough, float *d_slope, float *d_traver)
{
...
void Init_GPU_elevationmap(int length, float resolution, float h_mahalanobisDistanceThreshold_, float h_obstacle_threshold)
{
    ...
    cudaMalloc((void**)&h_map_intensity, sizeof(int) * length * length);
...
void Fuse(int length, int point_num, int *point_index, int *point_colorR, int *point_colorG, int *point_colorB, float *point_height, float *point_var, float *point_intensity)
{
	...
	int threadsPerBlock = 256; 
    int blocksPerGrid =(cell_num + threadsPerBlock - 1) / threadsPerBlock; 
    G_fuse<<<blocksPerGrid, threadsPerBlock>>>(dev_pointindex, dev_pointcolorR, dev_pointcolorG, dev_pointcolorB, dev_pointheight, dev_pointvar, dev_pointintensity, point_num); 
...
void Map_feature(int length, float *elevation, float *var, float *intensity, int *colorR, int *colorG, int *colorB, float *rough, float *slope, float *traver)
{
    float *d_elevation;
    float *d_intensity;
    float *d_var;
	...
	int threadsPerBlock = 256; 
	int blocksPerGrid =(cell_num + threadsPerBlock - 1) / threadsPerBlock; 
    G_Mapfeature<<<blocksPerGrid, threadsPerBlock>>>(d_colorR, d_colorG, d_colorB, d_elevation, d_var, d_intensity, d_rough, d_slope, d_traver); 
